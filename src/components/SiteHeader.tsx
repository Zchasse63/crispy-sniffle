  ...
  return (
    <header className="sticky top-0 z-40 border-b border-paper-line bg-paper-raise/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-2 px-3 sm:px-6">
        <Link href="/" className="flex shrink-0 items-center gap-2" aria-label="Scout home">
          <span className="text-ink">
            <SignalPin size={40} />
          </span>
          <span className="display text-xl tracking-wide text-ink sm:text-2xl">Scout</span>
        ...
        <nav className="flex min-w-0 items-center gap-0.5 sm:gap-2" aria-label="Primary">
          {NAV.map(({ href, label }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`readout rounded-md px-2 py-2 transition-colors sm:px-3 ${
          ...
          <button
            ...
            className="relative ml-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-paper-line bg-paper-raise text-ink transition-colors hover:border-ink/40 sm:ml-1 sm:h-10 sm:w-10"
          >
